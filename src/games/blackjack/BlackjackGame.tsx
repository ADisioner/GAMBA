...
export function BlackjackGame({ bet, luck, houseEdge, balance, onResult, multiplayer }: Props) {
  ...
  const deal = useCallback(() => {
    ...
    const winChance = applyLuck(0.04, luck)
  ...
  const stand = useCallback(async () => {
    ...
    const pScore = cardScore(playerHand); const dScore = cardScore(dH)
    console.log('Result check:', { pScore, dScore, playerHand, dH })
    
    if (dScore > 21) {
      setMessage(`Дилер перебрал! +${(bet * 2).toLocaleString()}`); setGameState('done')
      sounds.win(); onResult('win', bet * 2, { player: playerHand, dealer: dH })
      ...
      sounds.lose(); onResult('lose', 0, { player: playerHand, dealer: dH })
    }
  ...
  useEffect(() => {
    ...
    // 4. Сброс стола и вывод результата
    if (room?.status === 'ended') {
       ...
       if (!message) {
         if (pScore > 21) setMessage('Перебор! 💀')
         else if (dScore > 21) setMessage('Дилер перебрал! 🎉')
         else if (pScore > dScore) setMessage('Вы выиграли! 💸')
         else if (pScore === dScore) setMessage('Ничья! 🤝')
         else setMessage('Дилер выиграл 😞')
       }
  ...
  }, [isMulti, room?.status, room?.turn, profile?.nickname])
  ...
  return (
    <div className="relative p-4 flex flex-col items-center min-h-[600px] bg-emerald-900/20">
      {/* Table Background Decoration */}
      <div className="absolute inset-0 border-[12px] border-emerald-950/40 rounded-3xl pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] border-2 border-gold/5 rounded-[150px] pointer-events-none" />
        ...
        <div className="flex-1 w-full flex flex-col items-center justify-between py-6">
          ...
          <div className="relative w-full h-64 mt-auto">
             {[1,2,3,4,5,6].map(seatNum => {
                ...
                // Position calculation for semi-circle
                ...
                return (
                  <div key={seatNum} className="absolute left-1/2 bottom-10 -translate-x-1/2" 
          ...
          {/* Action Controls for Multiplayer */}
          <div className="h-16 flex items-center justify-center">
            {myPlayer && (
              <div className="flex gap-4">
                 {room.status === 'waiting' && myPlayer.status === 'ready' && (
                   <Button onClick={handleMultiBet} className="bg-gold hover:bg-gold-light text-velvet-dark font-bold px-8 shadow-glow-gold">
                      ПОСТАВИТЬ {formatBalance(bet)}
                   </Button>
                 )}
                 {room.turn === profile?.uid && room.status === 'playing' && (
                   <div className="flex gap-3">
                      <Button onClick={handleMultiHit} variant="outline" className="border-gold text-gold hover:bg-gold/10">Ещё (Hit)</Button>
                      <Button onClick={handleMultiStand} className="bg-gold text-velvet-dark font-bold">Хватит (Stand)</Button>
                   </div>
                 )}
              </div>
            )}
