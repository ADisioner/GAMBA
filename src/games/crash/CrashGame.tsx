...
export function CrashGame({ bet, luck, houseEdge, balance, onResult }: Props) {
  ...
  return (
    <div className="p-8 flex flex-col items-center">
      {/* Graph area */}
      <div className="w-full max-w-2xl h-80 sm:h-96 rounded-xl bg-marble border border-gold/20 flex items-center justify-center mb-6 relative overflow-hidden">
        {/* Background grid */}
        ...
        {state === 'idle' ? (
          <p className="text-muted-foreground text-lg">Нажмите «Старт» для начала</p>
        ) : (
          <motion.div animate={state === 'crashed' ? { scale: [1, 1.2, 0.9] } : {}}
            className="text-center">
            <p className={`text-7xl sm:text-8xl font-bold ${getColor()} transition-colors`}>
      ...
      <div className="flex gap-4 mb-6">
        {state === 'idle' || state === 'crashed' || state === 'cashed' ? (
          <Button onClick={start} size="xl" disabled={bet > balance} className="min-w-48 px-10">
            🚀 СТАРТ
          </Button>
        ) : (
          <Button onClick={cashOut} size="xl" variant="neon" className="min-w-48 w-auto px-10 animate-pulse">
            💰 ЗАБРАТЬ (${Math.floor(bet * multiplier).toLocaleString()})
          </Button>
        )}
      </div>
