...
export function GamePage() {
  ...
  const { room, loading: roomLoading, joinRoom, leaveRoom, placeBet, updatePlayerStatus } = useMultiplayerRoom(gameType, roomId)
  ...
  function adjustBet(delta: number) {
    setBet(prev => Math.max(minBet, Math.min(profile?.balance || 0, prev + delta)))
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className={`${roomId ? 'max-w-7xl' : 'max-w-5xl'} mx-auto px-4 py-6`}>
        {/* Bet controls - Only for Single Player */}
        {!roomId && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-wrap gap-1 ml-2 justify-center">
              ...
              <Button variant="ghost" size="sm" onClick={() => setBet(Math.max(minBet, Math.floor((profile?.balance || 0) * 0.1)))} className="text-xs px-2">10%</Button>
              <Button variant="ghost" size="sm" onClick={() => setBet(Math.max(minBet, Math.floor((profile?.balance || 0) * 0.33)))} className="text-xs px-2">33%</Button>
